...
namespace AF.Dungeon
{
    public sealed class DungeonGenerator : MonoBehaviour
    {
        ...
        public void Generate()
        {
            ...
            if (roomSize < 2)
            {
                Debug.LogError("[Dungeon Generator] roomSize must be at least 2");
                return;
            }


            int baseSeed = RunCoordinator.Instance != null
                ? RunCoordinator.Instance.Session.Seed
                : fallbackSeed;
            ...
            Dictionary<string, GameObject> prefabLookup = DungeonCatalogBuilder
                .BuildPrefabLookup(layoutSequence, defaultSideRoomCategory, connectorCategory);
            ...
            for (int i = 0; i < resultRooms.Count; i++)
            {
                ...
                roomObject.name = i == 0
                    ? $"{placed.Template.Id}_Start"
                    : $"{placed.Template.Id}_{i}";
        ...
        void CloseUnusedConnections()
        {
            ...
            foreach (GameObject roomObject in spawnedRooms)
            {
                ...
                foreach (DoorExit exit in exits)
                {
                    ...
                    {
                        GameObject blocker = Instantiate(deadEndPrefab, exit.transform.position, exit.transform.rotation, transform);
                ...
                foreach (DoorEntrance entrance in entrances)
                {
                    ...
                    {
                        Quaternion rotation = entrance.transform.rotation * Quaternion.Euler(0, 180f, 0);

                        GameObject blocker = Instantiate(deadEndPrefab, entrance.transform.position, rotation, transform);
